...
cv2.createTrackbar('RPM_value', 'sliders', rpm_neutral, rpm_forward, callback)
...
def camera_values(data):
    ...
    rpm_input = cv2.getTrackbarPos('RPM_value', 'sliders')
    ...
    rpm_int = int(rpm_input)

    steering_pub.publish(steering_float)
    rpm_pub.publish(rpm_int)
...
if __name__ == '__main__':
    ...
    rpm_pub = rospy.Publisher(RPM_REQUEST_TOPIC_NAME, Int32, queue_size=1)
    rate = rospy.Rate(60)
